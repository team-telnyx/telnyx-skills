...
1. Buy a phone number (see telnyx-numbers-python)
2. Create a messaging profile and configure webhook URL (see telnyx-messaging-profiles-python)
3. Assign the phone number to the messaging profile
4. For US A2P via long code: complete 10DLC registration — brand, campaign, number assignment (see telnyx-10dlc-python)
5. For toll-free: complete toll-free verification

### Steps

1. **Search & buy number**: `client.available_phone_numbers.list() → client.number_orders.create()`
...
4. **Send SMS**: `client.messages.send(from_=..., to=..., text=...)`
5. **Send MMS**: `client.messages.send(from_=..., to=..., text=..., media_urls=['https://...'])`
...
**Related skills**: telnyx-messaging-profiles-python, telnyx-10dlc-python, telnyx-numbers-python
